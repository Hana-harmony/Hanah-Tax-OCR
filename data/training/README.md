...
- `field_crops/`: field-level crop images and manifests for recognizer fine-tuning
  ...
  and split summaries include unique source counts so source leakage is visible during review,
...
- `recognizer/`: PaddleOCR recognizer train/val label files, per-group dictionaries, and plan files
  ...
  source-diversity counts and warnings, any capped hard-case counts applied during train split balancing,
