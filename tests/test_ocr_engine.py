...
def test_paddle_ocr_engine_prefers_english_middle_name_alternate_on_tie(
    tmp_path: Path,
) -> None:
    image_path = tmp_path / "sample.png"
    Image.new("RGB", (100, 100), "white").save(image_path)
    ...
    class FakePaddleOCR:
        ...
        def ocr(self, image_input: object, cls: bool = True) -> list[list[object]]:
            ...
            height = len(image_input)  # type: ignore[arg-type]
            if width >= 10 and height >= 6:
                text = "L" if self.kwargs.get("lang") == "en" else "M"
            else:
                return [[]]
            return [[
                (
    ...
    assert [kwargs["lang"] for kwargs in init_kwargs] == ["en", "korean"]
    assert regions["middle_name"].raw_text == "L"
